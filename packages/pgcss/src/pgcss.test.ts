...
describe('pgcss with real PGlite', () => {
	...
	describe('initSchema', () => {
      ...
      `)
			expect(tables[0].rows.map((r) => r.tablename)).toEqual(
				expect.arrayContaining(['selectors', 'declarations']),
			)
		})

		it('should create tables with custom names', async () => {
			const config = {
				selectorsTable: 'my_selectors',
				declarationsTable: 'my_declarations',
			}
      ...
      `)
			expect(tables[0].rows.map((r) => r.tablename)).toEqual(
				expect.arrayContaining(['my_selectors', 'my_declarations']),
			)
		})

		it('should throw an error for invalid table names', async () => {
			const invalidConfig = { selectorsTable: 'invalid-table' }
			await expect(initSchema(db, invalidConfig)).rejects.toThrow(
				'Invalid table name',
			)
		})
	})
	...
	describe('subscribe', () => {
      ...
      `)
			...
			// Check for updates
			await db.exec(
				`INSERT INTO declarations (selector_id, property, value) VALUES (1, 'background', 'blue');`,
			)
